...
export default async function ProducePage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-20 md:py-28">
      <Reveal>
        <div className="rounded-2xl bg-gradient-to-br from-surface/15 to-surface/5 p-10 md:p-16">
          <span className="inline-block rounded-full bg-surface/20 px-4 py-1 text-xs font-semibold uppercase tracking-widest text-surface">Market</span>
          <h1 className="mt-4 font-serif text-4xl md:text-5xl text-surface">Our Produce</h1>
          <p className="mt-4 max-w-xl text-lg text-surface/75">
      ...
      {listings.length === 0 && (
        <div className="mt-20 text-center">
          <p className="text-surface/55">No produce available right now. Check back soon.</p>
        </div>
      )}
    </div>
  );
